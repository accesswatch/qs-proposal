...
export default async function () {
    ...
    const addUrlIfAllowed = (candidate) => {
        const normalized = normalizeUrl(candidate);
        if (!normalized)
            return;
        if (!isAllowedUrl(normalized, domainAllowList, options.excludePatterns))
            return;
    ...
    const urlConfigs = urls.map(url => ({ url }));
...
function loadOptions() {
    ...
    const excludePatterns = parseExcludePatterns(core.getMultilineInput('exclude_url_patterns', { required: false }).map(v => v.trim()).filter(Boolean));
    ...
    const maxSitemapFiles = parsePositiveInt(core.getInput('max_sitemap_files', { required: false }) || '50', 'max_sitemap_files');
    const maxWordpressPages = parsePositiveInt(core.getInput('max_wordpress_pages', { required: false }) || '20', 'max_wordpress_pages');
    const requestTimeoutMs = parsePositiveInt(core.getInput('request_timeout_ms', { required: false }) || '10000', 'request_timeout_ms');
    return {
        seedUrls,
        sitemapUrls,
...
function buildDomainAllowList(options) {
    if (options.includeDomains.length > 0)
        return options.includeDomains.map(v => v.toLowerCase());
    const discoveredDomains = new Set();
    for (const raw of [...options.seedUrls, ...options.sitemapUrls]) {
        const hostname = safeHostname(raw);
        if (hostname)
            discoveredDomains.add(hostname.toLowerCase());
    }
    return [...discoveredDomains];
}
async function discoverFromSitemaps(sitemapUrls, options) {
...
async function discoverFromDrupal(drupalRoutesUrl, options) {
    const foundUrls = new Set();
    try {
        const body = await fetchText(drupalRoutesUrl, options);
        const parsed = JSON.parse(body);
        for (const candidate of collectUrlCandidates(parsed))
...
async function discoverFromWordPress(wordpressBaseUrl, options) {
    const foundUrls = new Set();
        ...
        for (let page = 1; page <= options.maxWordpressPages; page++) {
            ...
            try {
                ...
                for (const item of parsed) {
                    if (item.status === 'publish' && item.link)
                        foundUrls.add(item.link);
                }
            }
            catch (error) {
                if (String(error).includes('status 400') || String(error).includes('status 404'))
                    break;
...
async function crawlFromSeeds(seedUrls, options, domainAllowList) {
    const foundUrls = new Set();
    ...
    while (queue.length > 0 && foundUrls.size < options.maxUrls) {
        ...
        try {
            ...
            for (const link of links) {
                ...
                if (!normalizedLink)
                    continue;
                if (!isAllowedUrl(normalizedLink, domainAllowList, options.excludePatterns))
                    continue;
...
export function collectUrlCandidates(value) {
    const urls = new Set();
    collectRecursive(value, urls);
    return [...urls];
}
function collectRecursive(value, out) {
...
function normalizeMaybeRelativeUrl(value) {
...
function isAllowedUrl(url, domainAllowList, excludePatterns) {
